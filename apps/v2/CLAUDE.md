...
- **Receipt and inspection are split into two separate moments** (per Robert's working practice — goods are accepted quickly, then checked later at a quiet moment, often at the start of spraying):
  - **`bevestigd` → `ontvangen` = receipt**: a single tap "Goederen ontvangen" on `OrderDetail.vue` (`ontvangstBevestigen()`) — **no questions asked**, because whoever accepts the delivery (often an intern/bookkeeper, or a spuiter mid-spray) has no time. It only flips the status; the customer is told the goods arrived but **no completion date is promised** (the spuiter hasn't started yet — promising one would be false hope). The arrival photo is **optional** (a secondary button in the `ontvangen` state). `markBeforeFoto` keeps a fallback `bevestigd → ontvangen` so taking the photo first also counts as receipt.
  - **`ontvangen` → `in_productie` = Controle** (`Controle.vue`): later, at the start of spraying, the spuiter checks colour and panel counts and sets the lead time (default 15 workdays, only adjusted if it deviates) — this is where the "klaar over X werkdagen" date is established, because work is actually starting now. Confirming starts production. Deviations are reported via Meerwerk.
